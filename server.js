...
async function getAccessToken() {
  ...
  console.log('[Auth] ✓ Access Token erneuert, warte 2s...');
  await new Promise(r => setTimeout(r, 2000));
  console.log('[Auth] ✓ Bereit');
  return cachedAccessToken;
}
...
function getMonatOrdner(datumStr) {
  ...
  return `${d.getFullYear()}/${mm}-${MONATE[d.getMonth()]}`;
}
...
function getKundenOrdner(vorname, nachname, datumStr) {
  ...
  // Umlaute ersetzen, Kleinbuchstaben → Groß-/Kleinschreibung egal
...
async function dropboxRequest(buffer, dropboxPath, mode) {
  ...
  const apiArg = { path: dropboxPath, mode, mute: false };
  if (mode === 'add') apiArg.autorename = true;
  const apiArgStr = JSON.stringify(apiArg);

  console.log(`[Dropbox] Upload → ${dropboxPath}`);
  console.log(`[Dropbox] Token-Prefix: ${token.slice(0,25)}...`);
  console.log(`[Dropbox] API-Arg: ${apiArgStr}`);
  console.log(`[Dropbox] Buffer-Größe: ${buffer.length} bytes`);
  ...
  console.log(`[Dropbox] Response ${res.status}: ${body.slice(0, 200)}`);

  if (!res.ok) throw new Error(`Dropbox ${res.status}: ${body}`);
...
app.post('/api/upload', upload.array('files'), async (req, res) => {
  try {
    ...
    const base = `/Vorlagen ${artist}/${getMonatOrdner(datum)}/${getKundenOrdner(vorname, nachname, datum)}`;
    const uploadedItems = [];
